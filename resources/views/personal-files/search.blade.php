@extends('layouts.app')

@section('title')
    @parent Поиск
@endsection

@section('personal-files.menu')
    @include('personal-files.menu')
@endsection

@section('admin.menu')
    @include('admin.menu')
@endsection

@section('content')
    <div class="py-5">
        <h2><strong>Поиск анкеты</strong></h2>
    </div>

    <p class="fs-5">Введите серию и номер паспорта без пробелов</p>
    <form class="d-flex" method="POST" action="{{ route('personal-file.edit-search') }}">
        @csrf
        <div class="col-3 me-4">
            <input
                class="form-control"
                aria-label="Поиск"
                name="search"
                value=""
                type="search"
                {{--required--}}
                placeholder="Поиск">
        </div>
        <div>
            <button
                class="btn btn-primary px-5"
                type="submit">Поиск
            </button>
        </div>
    </form>

    @if(!empty($student))
        <div class="pt-5">
            <table class="table table-bordered">
                <thead>
                <tr class="table-primary">
                    <th scope="col" class="col-2 text-center">Номер дела</th>
                    <th scope="col" class="col-3 text-center">Фамилия</th>
                    <th scope="col" class="col-3 text-center">Имя</th>
                    <th scope="col" class="col-3 text-center">Отчество</th>
                    <th scope="col" class="col-3 text-center"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                        </svg></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <th scope="row" class="col-2 text-center">{{ $student->id }}</th>
                    <td class="col-3 text-center">{{ $student->surname }}</td>
                    <td class="col-3 text-center">{{ $student->name }}</td>
                    <td class="col-3 text-center">{{ $student->patronymic }}</td>
                    <td class="col-3 text-center"><a href="#">Редактировать</a></td>
                </tr>
                </tbody>
            </table>
        </div>
    @elseif(isset($student))
        <div>
            <p class="pt-5 fs-5 text-danger">Ничего не найдено</p>
        </div>
    @endif
@endsection
